    ...
    public function index(Request $request)
    {
        $viewData = $this->renderCrud($request, function (GroceryCrud $crud) {
            ...
            if (!Schema::hasColumn('users', 'created_at'))
                $cols = array_diff($cols, ['created_at']);
            if (!Schema::hasColumn('users', 'email_verified_at'))
                $cols = array_diff($cols, ['email_verified_at']);
            ...
            $crud->setActionButton(
                ...
                function ($row) {
                return route('admin.users.view', $row->id);
            }
                ,
                false
            );
            ...
            if (Schema::hasTable('plans')) {
                ...
                if ($display)
                    $crud->setRelation('plan_id', 'plans', $display);
            }
            ...
            if (Schema::hasColumn('users', 'email_verified_at')) {
                $crud->callbackColumn('email_verified_at', function ($value) {
                    ...
                    // Normalizaciones + Validation + Hash de password
